...
```
- **No API key required** — the running app makes no LLM calls. `backend/.env` is only for
  optional paid directions providers; the default OSRM commute provider needs no key.
...
- **The app makes NO live LLM calls.** It is a pure data layer. All LLM work — auto-scoring
  listings, extracting a pasted listing into structured fields — is done in a Claude Code session
  that writes results directly into db.json (the old `/listings/extract` intake endpoint was
  removed). Listings carry a special `auto` rater (Claude's 1–5 scores); `combined_score` falls
  back to `auto` when no human has rated, and `ranked_by` reports `human`/`auto`/`none`.

- **Data-maintenance primitives** in `backend/scripts/` must run **on a VPN** (Craigslist
  IP-blocks scraping; it returns HTTP **410** for expired posts). All are idempotent, throttled,
  hard-stop on the first 403/429 or block page, and back up db.json first:
  ...
  - `dedupe_listings.mjs` — cluster reposts and mark all but one with `duplicate_of`.
  - Typical cycle: **collect → refresh → dedupe → auto-score new (in Claude Code) → commit + push**.
...
- **Static-deploy duality.** `frontend/src/api.ts` swaps between `liveApi` (`/api` → backend) and
  ...
  persist per-browser to localStorage and backend-only affordances (CSV export, live commute) are
  hidden.
...
- **`README.md` is partially stale** — it predates this work and still describes a live LLM intake
  endpoint, a required Anthropic key, and "not a scraper." Trust this file and the code over it.
